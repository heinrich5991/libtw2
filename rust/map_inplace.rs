...
impl<T,U> PartialVec<T,U> {
	...
	pub fn new(mut vec: Vec<T>) -> PartialVec<T,U> {
		...
		// This `as int` cast is safe, because the size of the elements
		// of the vector is not 0, and:
		//
		// 1) If the size of the elements in the vector is 1, the `int`
		//    may overflow, but it has the correct bit pattern so that
		//    the `.offset()` function will work.
		//
		//    Example:
		...
		//        (0x1 + 0x8 = 0x1 - 0x8)
		//
		// 2) If the size of the elements in the vector is >1, the
		//    `uint` -> `int` conversion can't overflow.
		...
		// `start_u == end_u == start_t <= end_t`, so also `start_u <=
		// end_u <= start_t <= end_t`, thus (b).

		// As `start_u == end_u`, it is represented correctly that
		// there are no instances of `U` in `vec`, thus (e) is
		// satisfied.

		// At start, there are only elements of type `T` in `vec`, so
		// (f) is satisfied, as `start_t` points to the start of `vec`
		// and `end_t` to the end of it.

		// This points inside the vector, as the vector has length
		// `offset`.

		PartialVec {
			// (a) is satisfied, `vec` isn't modified in the
			// function.
			vec: vec,
			start_u: start_u,
			end_u: end_u,
	...
	fn pop(&mut self) -> Option<T> {
		...
		if self.start_t < self.end_t {
			let result;
			unsafe {
				// (f) is satisfied before, so in this if
				// branch there actually is a `T` at `start_t`.
				// After shifting the pointer by one, (f) is
				// again satisfied.
	...
	/// # Failure
	///
	/// Fails if not enough `T`s were popped to have enough space for the
	/// new `U`.
	...
	/// # Failure
	///
	/// Fails if not all `T`s were popped, also fails if not the same
	/// amount of `U`s was pushed before calling `unwrap`.
	pub fn unwrap(self) -> Vec<U> {
		// If `self.end_u == self.end_t`, we know from (e) that there
		// are no more `T`s in `vec`, we also know that the whole
		// length of `vec` is now used by `U`s, thus we can just
		// transmute `vec` from a vector of `T`s to a vector of `U`s
		// safely.

		assert!(self.end_u as *() == self.end_t as *(),
			"trying to unwrap a PartialVec before completing the writes to it");

		// Extract `vec` and prevent the destructor of `PartialVec`
		// from running.
		unsafe {
...
impl<T,U> Drop for PartialVec<T,U> {
	fn drop(&mut self) {
		unsafe {
			// As per (a) `vec` hasn't been modified until now. As
			// it has a length currently, this would run
			// destructors of `T`s which might not be there. So at
			// first, set `vec`s length to `0`. This must be done
			// at first to remain memory-safe as the destructors of
			// `U` or `T` might cause unwinding where `vec`s
			...
			// will run, deallocating the underlying memory.
		}
	}
}
