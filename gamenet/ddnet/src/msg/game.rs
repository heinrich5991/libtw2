...
impl SvTuneParams {
    pub fn decode<W: Warn<Warning>>(warn: &mut W, _p: &mut Unpacker) -> Result<SvTuneParams, Error> {
        let result = Ok(SvTuneParams {
            ...
            jetpack_strength: TuneParam(_p.read_int(warn).unwrap_or(40000)),
            shotgun_strength: TuneParam(_p.read_int(warn).unwrap_or(1000)),
            explosion_strength: TuneParam(_p.read_int(warn).unwrap_or(600)),
            hammer_strength: TuneParam(_p.read_int(warn).unwrap_or(100)),
            hook_duration: TuneParam(_p.read_int(warn).unwrap_or(125)),
            hammer_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(12500)),
            gun_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(12500)),
            shotgun_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(50000)),
            grenade_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(50000)),
            laser_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(80000)),
            ninja_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(80000)),
            hammer_hit_fire_delay: TuneParam(_p.read_int(warn).unwrap_or(32000)),
            ground_elasticity_x: TuneParam(_p.read_int(warn).unwrap_or(0)),
            ground_elasticity_y: TuneParam(_p.read_int(warn).unwrap_or(0)),
        });
        _p.finish(wrap(warn));
        result
    }
